...
@login_required
@required_roles('view_stock', 'view_it')
def stock():
    if request.method == "POST":
        if 'add-cat' in request.form:
            ...
            if categ_qu is None: # Check if Category is exists
...
# View Stock order items page
#==================================
@stock_routes.route("/view-order/<int:order_id>" , methods=['GET', 'POST'])
@login_required
def stock_order_view(order_id):
    # Join Tables to get all items details
    items = db.session.query(Stock, Orders, Stock_cat).join(Orders).join(Stock_cat).filter(Orders.id == order_id, Stock_cat.id ==  Stock.stock_categ).all()
        
    return render_template('/stock/stock-view-order.html', title="Order Items",stockinout="active", order_id= order_id, items = items)
...
def stock_add_order(order_id):
    if request.method == 'POST':
        ...
        for category, name, serial, quantity in zip(item_category, item_name, item_serial, item_quantity):
            ...
            get_stock_item = Stock(item_name = name, serial = serial, quantity = quantity,stock_categ = int(category), order_quantity = quantity, stock_order = int(order_id))
