...
from uxapp.models import pcs, Pcs_categ
...
# PCs Details Route
#=========================
@pcs_routes.route("/pcdetails/<int:categ>" , methods=['GET', 'POST'])
@login_required
def pcdetails(categ):
    ...
    if request.method == "POST":
        if 'addpc' in request.form:
            ...
            try:
                ...
                return redirect(url_for('pcs_routes.pcdetails'))
    pcs_table =  pcs.query.join(Pcs_categ).filter(pcs.pcs_category == categ).all() 
...
#=========================        
# PCs Categories Route
#=========================
@pcs_routes.route("/pcs-categories" , methods=['GET', 'POST'])
@login_required
def pcs_cate():
    pcs_cates = Pcs_categ.query.order_by(Pcs_categ.id)
    return render_template("/pcs/pcs-categories.html", title="PCs Categories", pcs_cates = pcs_cates)